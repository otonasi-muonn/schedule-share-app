...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _updateSchedule({
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('予定を更新しました。')));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('更新中にエラーが発生しました: $e')));
      }
    }
  }
  ...
  Future<void> _showScheduleDialog({DocumentSnapshot? scheduleDoc}) async {
    ...
    if (isEditing) {
      initialData = scheduleDoc.data() as Map<String, dynamic>;
    }
    
    final titleController = TextEditingController(text: isEditing ? initialData!['title'] : '');
    DateTime selectedDate = isEditing ? (initialData!['startTime'] as Timestamp).toDate() : DateTime.now();
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                onTap: () {
                  // 編集ダイアログを呼び出す
                  _showScheduleDialog(scheduleDoc: doc);
                },
