...
import 'package:schedule_share_app/presentation/friends/friends_hub_screen.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('ホーム（月間）'), actions: [
        IconButton(icon: const Icon(Icons.timeline), tooltip: 'タイムラインビュー', onPressed: () {
          Navigator.of(context).push(MaterialPageRoute(builder: (context) => TimelineViewScreen(initialDate: _selectedDay ?? DateTime.now())));
        }),
  ...
  Widget buildCalendarAndList(List<QueryDocumentSnapshot> allDocs) {
    ...
    final filteredDocs = allDocs.where((doc) {
      ...
      ..sort((a, b) {
        ...
        final aTime = aData.containsKey('startTime') ? aData['startTime'] as Timestamp? : null;
        final bTime = bData.containsKey('startTime') ? bData['startTime'] as Timestamp? : null;
