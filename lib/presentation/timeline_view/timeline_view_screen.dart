...
class RenderableEvent {
  ...
  int column;
  int totalColumns;
  RenderableEvent({required this.doc, required this.displayStart, required this.displayEnd, this.column = 0, this.totalColumns = 1});
}
...
class _TimelineViewScreenState extends State<TimelineViewScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('schedules').where('userId', isEqualTo: user?.uid).where('startTime', isGreaterThanOrEqualTo: _startDate).where('startTime', isLessThan: _startDate.add(Duration(days: _totalDays))).orderBy('startTime').snapshots(),
        builder: (context, snapshot) {
          ...
          return Stack(
            children: [
              ...
              ..._buildEventWidgets(allRenderableEvents),
  ...
  Widget _buildBackgroundGrid() {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Row(
          children: [
            SizedBox(width: 60, height: _hourHeight, child: Center(child: Text('${hour.toString().padLeft(2, '0')}:00', style: const TextStyle(fontSize: 12)))),
            Expanded(
              child: Container(
  ...
  List<Widget> _buildEventWidgets(List<RenderableEvent> renderableEvents) {
    ...
    return renderableEvents.map((event) {
      ...
      return Positioned(
        ...
        left: left, width: columnWidth, height: height,
